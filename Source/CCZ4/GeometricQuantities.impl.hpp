...
//my ugly stuff        
    ...
    {
        delete m_riemann_squared_T;
        m_riemann_squared_T = nullptr;
    }
    if (m_riemann_LLLU_ST_T != nullptr)
    {
        delete m_riemann_LLLU_ST_T;
        m_riemann_LLLU_ST_T = nullptr;
    }
    if (m_riemann_LULU_ST_T != nullptr)
    {
...
/*My ugly stuff*/
template_GQ const Tensor<4, data_t, CH_SPACEDIM + 1> &
GeometricQuantities_t::get_riemann_LLLL_ST_T()
{
    assert_with_label(m_formulation >= 0, m_label); // formulation is set
    assert_with_label(GR_SPACEDIM == 3, m_label);
    if (m_riemann_LLLL_ST_T == nullptr)
        compute_riemann_LLLL_ST_T();
    return *m_riemann_LLLL_ST_T;
}
/*My ugly stuff*/
template_GQ const Tensor<4, data_t, CH_SPACEDIM + 1> &
...
template_GQ const data_t &GeometricQuantities_t::get_ricci_scalar_ST_T()
{
    assert_with_label(m_formulation >= 0, m_label); // formulation is set
    if (m_ricci_scalar_ST_T == nullptr)
        compute_ricci_scalar_ST_T();
    return *m_ricci_scalar_ST_T;
}

template_GQ const data_t &GeometricQuantities_t::get_ricci_squared_T()
{
    assert_with_label(GR_SPACEDIM == 3, m_label);
    if (m_ricci_squared_T == nullptr)
        compute_ricci_squared_T();
    return *m_ricci_squared_T;
}

template_GQ const data_t &GeometricQuantities_t::get_kretschmann_T()
{
    assert_with_label(m_formulation >= 0, m_label); // formulation is set
    assert_with_label(GR_SPACEDIM == 3, m_label);
    if (m_kretschmann_T == nullptr)
...
/*My ugly stuff*/
template_GQ const data_t &GeometricQuantities_t::get_riemann_squared_T()
{
    assert_with_label(m_formulation >= 0, m_label); // formulation is set
    assert_with_label(GR_SPACEDIM == 3, m_label);
    if (m_riemann_squared_T == nullptr)
        compute_riemann_squared_T();
    return *m_riemann_squared_T;
}
template_GQ const Tensor<4, data_t, CH_SPACEDIM + 1> &
GeometricQuantities_t::get_riemann_LLLU_ST_T()
{
    assert_with_label(m_formulation >= 0, m_label); // formulation is set
    assert_with_label(GR_SPACEDIM == 3, m_label);
    if (m_riemann_LLLU_ST_T == nullptr)
        compute_riemann_LLLU_ST_T();
    return *m_riemann_LLLU_ST_T;
}
template_GQ const Tensor<4, data_t, CH_SPACEDIM + 1> &
GeometricQuantities_t::get_riemann_LULU_ST_T()
{
    assert_with_label(m_formulation >= 0, m_label); // formulation is set
    assert_with_label(GR_SPACEDIM == 3, m_label);
    if (m_riemann_LULU_ST_T == nullptr)
        compute_riemann_LULU_ST_T();
...
/*my ugly stuff*/
template_GQ void GeometricQuantities_t::compute_riemann_LLLL_ST_T()
{
    if (m_riemann_LLLL_ST_T != nullptr)
        delete m_riemann_LLLL_ST_T;
    ...
    m_riemann_LLLL_ST_T = new Tensor<4, data_t, CH_SPACEDIM + 1>;
    FOR_ST(a, b, m, n)
    {
        (*m_riemann_LLLL_ST_T)[a][b][m][n] =
            weyl[a][b][m][n] +
            (g[a][m] * ricci[n][b] - g[a][n] * ricci[m][b] -
             g[b][m] * ricci[n][a] + g[b][n] * ricci[m][a]) /
                (GR_SPACEDIM - 1.) -
...
// my ugly stuff
template_GQ void GeometricQuantities_t::compute_ricci_squared_T()
{
    if (m_ricci_squared_T != nullptr)
        delete m_ricci_squared_T;
...
template_GQ void GeometricQuantities_t::compute_riemann_squared_T()
{
    ...
    const auto &riemann_LULU = get_riemann_LULU_ST_T();

    m_riemann_squared_T = new data_t(0.);
    FOR_ST(a, b, c, d)
    {
        (*m_riemann_squared_T) +=
            riemann_LULU[a][b][c][d] * riemann_LULU[b][a][d][c];
...
template_GQ void GeometricQuantities_t::compute_riemann_LLLU_ST_T()
{
    if (m_riemann_LLLU_ST_T != nullptr)
        delete m_riemann_LLLU_ST_T;

    const auto &g_UU = get_metric_UU_ST();
    const auto &riemann_LLLL = get_riemann_LLLL_ST_T();

    m_riemann_LLLU_ST_T = new Tensor<4, data_t, CH_SPACEDIM + 1>({0.});
    FOR_ST(a, b, c, d, e)
    {
        (*m_riemann_LLLU_ST_T)[a][b][c][d] +=
            riemann_LLLL[a][b][c][e] * g_UU[e][d];
    }
}
template_GQ void GeometricQuantities_t::compute_riemann_LULU_ST_T()
{
    if (m_riemann_LULU_ST_T != nullptr)
        delete m_riemann_LULU_ST_T;

    const auto &g_UU = get_metric_UU_ST();
    const auto &riemann_LLLU = get_riemann_LLLU_ST_T();

    m_riemann_LULU_ST_T = new Tensor<4, data_t, CH_SPACEDIM + 1>({0.});
    FOR_ST(a, b, c, d, e)
    {
        (*m_riemann_LULU_ST_T)[a][b][c][d] +=
            riemann_LLLU[a][e][c][d] * g_UU[e][b];
    }
}
//my ugly stuff
