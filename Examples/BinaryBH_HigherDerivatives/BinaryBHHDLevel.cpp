...
void BinaryBHHDLevel::computeDiagnostics()
{
    ...
    // diffusion(
    //     c2eft, m_p.ccz4_params_modifiedGauge, m_p.diffusion_params, m_dx,
    //     m_dt, m_p.sigma, m_p.center, m_p.formulation, m_p.G_Newton,
                                      ...
                                      const double a_time)
{
    ...
    {
        MatterCCZ4RHSWithDiffusion<C2EFT<System>,
                                   MovingPunctureGaugeEtaRadialDecay,
                                   FourthOrderDerivatives>
            my_ccz4_matter(c2eft, m_p.ccz4_params_modifiedGauge,
                           m_p.diffusion_params, m_dx, m_dt, m_p.sigma,
...
void BinaryBHHDLevel::specificPostTimeStep()
{
    ...
    if (m_p.calculate_constraint_norms)
    {
        ...
        System EBsystem(m_p.system_params);
        C2EFT<System> c2eft(EBsystem, m_p.hd_params, apply_weak_field);
        MatterConstraintsWithGauge<C2EFT<System>,
                                   MovingPunctureGaugeEtaRadialDecay>
            constraints(c2eft, m_dx, m_p.G_Newton, m_p.formulation,
                        m_p.ccz4_params_modifiedGauge, m_p.center, c_Ham,
                        Interval(c_Mom, c_Mom));
