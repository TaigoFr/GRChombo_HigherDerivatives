...
if not plot:
    ...
    def _vol_eff(field, data):
        print("Starting _vol_eff", len(data["C"]))
        unit = file.length_unit.to('code_length')
        print("Computing points")
        points = np.transpose([data["index", "x"] - unit * puncture[0], data["index", "y"] - unit * puncture[1], data["index", "z"] - unit * puncture[2]])
        print("Computing norms")
        points_norm = [np.linalg.norm(point) for point in points]
        print("Computing invalid points")
        points_invalid = np.array([norm < AH_radius_vs_time(time) and norm > width for norm in points_norm])
    ...
    for i in range(0, len(ds), jump):
        ...
        point_max = puncture / radius * (radius +  width * 1.5)
        point_min = puncture / radius * (radius -  width * 1.5)
        print(point_min, point_max)
        domain = file.r[point_min[0]:point_max[0], point_min[1]:point_max[1], point_min[2]:max(point_max[2], width * 1.5)]
        ...
        allAverageCminusCphysWithoutAH.append([float(time), L2CvsCphys / L2C * 100])
    ...
    np.save('data_volume.npy', [times_C, C, Cphys, CvsCphys])
    exit()

else:
    results = np.load('data_volume.npy', allow_pickle=True)
    print(results)
    [times_C, C, Cphys, CvsCphys] = results
    ...
    do_plot(times_C, C, Cphys, False, "")
    do_plot(times_C, C, Cphys, True, "_log")
    do_plot(times_C, CvsCphys, [], False, "_diff")
