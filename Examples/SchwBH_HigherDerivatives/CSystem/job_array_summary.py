...
if len(sys.argv)>1:
    ...
    for i in range(total):
        specific_params = open(folders[i] + "/specific_params.txt")
        lines = specific_params.readlines()
        specific_params.close()
        ...
        if path.exists(folders[i] + "/stats_AH1.dat"):
            AH_stats = open(folders[i] + "/stats_AH1.dat")
            lines = AH_stats.readlines()
